...
try {
  ...
  console.warn("user badge failed", e);
}
...
try {
  ...
  if (curRaw) {
    ...
    if (rawAnalyst) {
      ...
      if (Array.isArray(analyst)) {
        const mine = analyst.filter((t) =>
          (t.user || "").toLowerCase().includes((cur.name || "").toLowerCase())
        );
        if (mine && mine.length) {
          tickets = mine.map((t) => ({
            id: t.id,
  ...
  console.warn("Failed to map analyst tickets for user view", e);
}
...
function renderTables() {
  ...
  tickets
    ...
    .forEach((t) => {
      ...
      const actionButton = (t.status === 'Aberto' || t.status === 'Em Andamento')
        ? `<button class="ghost" style="font-size:12px;padding:6px 8px;" onclick="cancelTicket(${t.id})">Cancelar</button>`
        : '';
  ...
  tickets.forEach((t) => {
    ...
    tr.innerHTML = `
      ...
      <td>${t.titulo}${
      reporter
        ? `<div class="muted" style="font-size:12px">${reporter}</div>`
        : ""
    }</td>
      <td>${t.categoria}</td>
      <td><span class="badge ${urgClass(t.urg)}">${t.urg}</span></td>
  ...
  tickets
    ...
    .forEach((t) => {
      ...
      const satisfContent = `<button class="btn-evaluate" onclick="evaluateTicket(${t.id})">Avaliar</button>`;


      tr.innerHTML = `<td>${t.id}</td><td>${t.titulo}</td><td>${satisfContent}</td><td>${
        t.closed || "-"
      }</td>`;
...
function evaluateTicket(id) {
    const rating = prompt('Em uma escala de 1 a 5, qual sua nota para o atendimento deste chamado?');
